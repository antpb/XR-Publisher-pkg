 ...
 */
 function parseMatrixUri(uri: string) {
	...
	if (url.protocol === "matrix:") {
		...
		const sigil1 = SegmentToSigil[segments[0] as keyof typeof SegmentToSigil];
 ...
 */
export function Portal(model: PortalProps): JSX.Element {
				...
				<RigidBody
					...
					onCollisionEnter={() => {
						...
						if (url.protocol === "matrix:") {
							...
							if (typeof destination !== 'string' && 'mxid1' in destination) {
								window.location.href =
