...
export default class P2PCF extends EventEmitter {
	constructor(clientId = "", roomId = "", options = {}) {
		...
		console.log("workerUrl", options.workerUrl);
	...
	async start(props) {
		...
		let guestDefaultAvatar = defaultAvatar.length > 0 ? defaultAvatar : defaultVRM;
	...
	_pingIfInRoom(roomId) {
		...
		const apiUrl = `/wp-json/xr-publisher/v1/handle-heart-check/${postId}`;
	...
	// _sendHeartbeat() {
	// 	const postId = userData.currentPostId;
	// 	const apiUrl = `/wp-json/xr-publisher/v1/send-heartbeat/${postId}`;
	...
	// 	const apiUrl = `/wp-json/xr-publisher/v1/get-room-count/${postId}`;
	// 	return fetch(apiUrl, {
	// 		method: 'GET',
	// 		headers: {
	// 			'Content-Type': 'application/json',
	...
	// 		apiUrl = `/wp-json/xr-publisher/v1/add-room-count/${postId}`;
	// 	} else if(action === "subtract") {
	// 		actionNonce = userData.subtractNonce;
	// 		apiUrl = `/wp-json/xr-publisher/v1/subtract-room-count/${postId}`;
	// 	}
	...
	// 	const apiUrl = `/wp-json/xr-publisher/v1/update-room-count/${postId}`;
		...
		peer.on("connect", () => {
			...
			if( roomCount < limit ) {
				...
				if(! validlyInRoom ) {
					...
					// example string #xr_publisher-room-1 take the value after the last - and cast it to int
